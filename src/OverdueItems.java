...
public class OverdueItems {
	...
	public void displayingOverdueItems()
	{
		...
		for (int i = 0; i < RentItem.rental.length; i++)
		{
			...
			if(RentItem.rental[i][1].equals("Weekly") && daysBorrowedINT > 7) //Show if the list of rented items is of weekly type
			{
				System.out.println(RentItem.rental[i][0] + "\t" + RentItem.rental[i][4] + "\t" + RentItem.rental[i][2]);
			}
			
			if(RentItem.rental[i][1].equals("Overnight") && daysBorrowedINT > 1) //Show if the list of rented items is of overnight type
			{
				System.out.println(RentItem.rental[i][0] + "\t" + RentItem.rental[i][4] + "\t" + RentItem.rental[i][2]);
			}
		}
